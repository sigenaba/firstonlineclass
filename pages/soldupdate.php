...
<div class="section-container">
    <div class="section-left">
        <form action="" method="post" class="thisform">
            <div class="selections"><label for="buyer">Buyer</label>
                <div class="selections">
                    <?php
                    $querybuyer = $con->query("SELECT * FROM accessories_sold WHERE system_id=$x") or die($con->error);
                    if (mysqli_num_rows($querybuyer) > 0) { ?>
                        ...
                        <?php } else { ?>
                            ...
                            <?php }
                        if (mysqli_num_rows($querybuyer) < 1) {
                            $result = $con->query("SELECT * FROM buyer") or die($con->query); ?>
                        ...
                        <?php } else {
                            $db_buyerid = $con->query("SELECT * FROM accessories_sold WHERE system_id=$x") or die($con->error);
                            while ($dbid = mysqli_fetch_assoc($db_buyerid)) {
                                $dbbuy = $dbid['buyer_id'];
                            }
                            $buyname = $con->query("SELECT * FROM buyer WHERE id=$dbbuy") or die($con->error);
                            while ($rowb = mysqli_fetch_assoc($buyname)) {
                                $buyee = $rowb['buyer_name'];
                                $buyid = $rowb['id'];
                            }
                        ?>
                            <option value='<?php echo $buyid; ?>' disable selected><?php echo $buyee; ?></option>
                        </select>
                </div>
            </div>
        <?php  } ?>
        <div class="selections"><label for="os">OS</label>
            <div class="selections">
                <select name="os" style="width: 50%;" id="selectdropdown17" class="form-control select2" class="option" required>
                    <?php
